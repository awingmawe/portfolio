...
export default function Projects() {
  ...
  const independentProjects = projects.filter(p => p.type === "independent");
    ...
    <section className="py-32 px-6 md:px-12 max-w-7xl mx-auto border-t border-border/50">
      ...
      <div className="flex flex-col mb-20">
        ...
        >
          ...
          <p className="text-accent uppercase tracking-[0.2em] text-xs font-semibold">Professional</p>
        </motion.div>
      </div>

      <div className="flex flex-col border-t border-border/50 mb-32">
      ...
      <div className="flex flex-col mb-20">
        <motion.h2 
          ...
          className="text-4xl md:text-5xl font-serif text-foreground mb-4"
        >
          Independent <span className="italic text-accent">Projects</span>
        </motion.h2>
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          <div className="w-12 h-[1px] bg-accent" />
          <p className="text-accent uppercase tracking-[0.2em] text-xs font-semibold">Open Source & Personal</p>
        </motion.div>
      </div>
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4">
              ...
              <div className="flex items-start gap-6 md:w-1/2">
                ...
                <h3 className="text-3xl md:text-5xl lg:text-6xl font-serif text-foreground">
                  {project.title}
                </h3>
              </div>
              ...
              <div className="md:w-1/3 flex flex-col items-start md:items-end gap-4">
                ...
                <div className="flex flex-wrap gap-2 md:justify-end">
                  {project.tags.map((tag, i) => (
                    <span 
                      key={i}
                      className="text-xs uppercase tracking-[0.1em] font-sans font-semibold text-foreground/40 transition-colors"
                    >
                      {tag} {i < project.tags.length - 1 && <span className="mx-1 opacity-30">/</span>}
                    </span>
                  ))}
